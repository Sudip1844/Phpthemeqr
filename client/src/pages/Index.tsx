...
const Index = ({ qrType }: IndexProps) => {
  ...
  // Redirect from root path (/) to /url-to-qr for better UX
  // SEO crawlers will still see root page SEO before redirect happens
  useEffect(() => {
    // Only redirect if we're on root path and no qrType is specified
    if (location.pathname === '/' && !qrType) {
      // Small delay to ensure SEO meta tags are indexed
      const redirectTimer = setTimeout(() => {
        navigate('/url-to-qr', { replace: true });
      }, 100);
      
      return () => clearTimeout(redirectTimer);
    }
  }, [location.pathname, qrType, navigate]);
